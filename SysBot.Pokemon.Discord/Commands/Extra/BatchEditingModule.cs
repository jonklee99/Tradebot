...
public class BatchEditingModule : ModuleBase<SocketCommandContext>
{
    ...
    {
        if (EntityBatchEditor.Instance.TryGetPropertyType(propertyName, out string? result))
    ...
    private static bool IsValidInstructionSet(ReadOnlySpan<char> split, out List<StringInstruction> invalid)
    {
        ...
        {
            if (!EntityBatchEditor.Instance.TryGetPropertyType(s.PropertyName, out string? _))
                invalid.Add(s);
        }
